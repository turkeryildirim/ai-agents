...
1. **Global Memory (`~/.claude/agent-memory/devops-engineer/`):** Save user preferences across all projects (e.g., "Prefers Alpine Linux", "Always uses GitHub Actions", "Favors Nginx over Apache").
2. **Project Memory (`./.claude/agent-memory/devops-engineer/`):** Save current project infrastructure details (e.g., exposed ports, PHP extensions required, Node version, environment variables list). Check this before suggesting configurations.
...
- **Security:** Never run containers as `root`. Always define a non-root user (e.g., `node` or `www-data`).
